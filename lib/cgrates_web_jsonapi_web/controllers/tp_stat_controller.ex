defmodule CgratesWebJsonapiWeb.TpStatController do
  ...
  def filter(_conn, query, "filter_ids", v),
    do: query |> where([s], like(s.filter_ids, ^"%#{v}%"))

  def filter(_conn, query, "activation_interval", v),
    do: query |> where([s], like(s.activation_interval, ^"%#{v}%"))
  ...
  def filter(_conn, query, "threshold_ids", v),
    do: query |> where([s], like(s.threshold_ids, ^"%#{v}%"))

  def filter(_conn, query, "metric_ids", v),
    do: query |> where([s], like(s.metric_ids, ^"%#{v}%"))

  def filter(_conn, query, "metric_filter_ids", v),
    do: query |> where([s], like(s.metric_filter_ids, ^"%#{v}%"))
