defmodule CgratesWebJsonapiWeb.TpActionController do
  ...
  def filter(_conn, query, "timing_tags", val),
    do: query |> where([r], like(r.timing_tags, ^"%#{val}%"))

  def filter(_conn, query, "destination_tags", val),
    do: query |> where([r], like(r.destination_tags, ^"%#{val}%"))

  def filter(_conn, query, "rating_subject", val),
    do: query |> where([r], like(r.rating_subject, ^"%#{val}%"))

  def filter(_conn, query, "categories", val),
    do: query |> where([r], like(r.categories, ^"%#{val}%"))

  def filter(_conn, query, "shared_groups", val),
    do: query |> where([r], like(r.shared_groups, ^"%#{val}%"))
