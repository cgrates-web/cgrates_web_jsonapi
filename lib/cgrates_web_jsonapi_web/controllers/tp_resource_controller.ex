defmodule CgratesWebJsonapiWeb.TpResourceController do
  ...
  def filter(_conn, query, "usage_ttl", val),
    do: query |> where([r], like(r.usage_ttl, ^"%#{val}%"))
  ...
  def filter(_conn, query, "threshold_ids", val),
    do: query |> where([r], like(r.threshold_ids, ^"%#{val}%"))
  ...
  def filter(_conn, query, "allocation_message", val),
    do: query |> where([r], like(r.allocation_message, ^"%#{val}%"))
