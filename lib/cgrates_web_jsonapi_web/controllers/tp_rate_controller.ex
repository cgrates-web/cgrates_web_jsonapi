defmodule CgratesWebJsonapiWeb.TpRateController do
  ...
  def filter(_conn, query, "connect_fee", connect_fee),
    do: query |> where(connect_fee: ^connect_fee)

  def filter(_conn, query, "rate_unit", rate_unit), do: query |> where(rate_unit: ^rate_unit)

  def filter(_conn, query, "rate_increment", rate_increment),
    do: query |> where(rate_increment: ^rate_increment)

  def filter(_conn, query, "group_interval_start", gis),
    do: query |> where(group_interval_start: ^gis)
