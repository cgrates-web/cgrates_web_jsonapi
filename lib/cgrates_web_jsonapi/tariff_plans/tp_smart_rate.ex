defmodule CgratesWebJsonapi.TariffPlans.TpSmartRate do
  use Ecto.Schema
  ...
  def changeset(struct, params \\ %{}) do
    ...
    |> validate_format(:rate_unit, CgratesWebJsonapi.TariffPlans.TpRate.time_format())
    |> validate_format(:rate_increment, CgratesWebJsonapi.TariffPlans.TpRate.time_format())
    |> validate_format(:group_interval_start, CgratesWebJsonapi.TariffPlans.TpRate.time_format())
    ...
    |> validate_inclusion(
      :rounding_method,
      CgratesWebJsonapi.TariffPlans.TpDestinationRate.rounding_methods()
    )
    |> validate_inclusion(
      :max_cost_strategy,
      CgratesWebJsonapi.TariffPlans.TpDestinationRate.max_cost_strategies()
    )
  end

  # TODO: Move to the context
  def destination_attrs(struct),
    do: %{prefix: struct[:prefix], tag: struct[:destination_tag], tpid: struct[:tpid]}
  ...
  def insert(changeset) do
    if changeset.valid? do
      ...
      %TpDestinationRate{}
      |> Map.merge(changes |> destination_rate_attrs())
      |> upsert_by([:tag, :tpid])

      %TpRatingPlan{}
      |> Map.merge(changes |> rating_plan_attrs())
      |> upsert_by([:tag, :destrates_tag, :tpid])
  ...
  def from_csv(path, tpid) do
    ...
    |> Enum.each(fn data ->
      %CgratesWebJsonapi.TariffPlans.TpSmartRate{}
  ...
  defp create_dst(changes) do
    ...
    if is_nil(dst) do
      %TpDestination{}
      |> TpDestination.changeset(changes |> destination_attrs())
      |> Repo.insert!()
