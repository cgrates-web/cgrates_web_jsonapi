defmodule CgratesWebJsonapi.TpDestinationRateControllerTest do
  ...
  test "lists all entries related tariff plan on index", %{conn: conn} do
    ...
    conn = get(conn, tp_destination_rate_path(conn, :index, tpid: tariff_plan_1.alias)) |> doc
  ...
  test "shows chosen resource", %{conn: conn} do
    ...
    conn = get(conn, tp_destination_rate_path(conn, :show, tp_destination_rate)) |> doc
  ...
  test "does not show resource and instead throw error when id is nonexistent", %{conn: conn} do
    assert_error_sent 404, fn ->
      get(conn, tp_destination_rate_path(conn, :show, -1)) |> doc
    end
  end
  ...
  describe "GET export_to_csv" do
    test "returns status 'ok'", %{conn: conn} do
      ...
      conn = conn
      |> get(tp_destination_rate_path(conn, :export_to_csv), %{tpid: tariff_plan.alias, filter: %{destinations_tag: "desttag1", tag: "drtag1"}})
      |> doc()
  ...
  test "creates and renders resource when data is valid", %{conn: conn} do
    ...
    conn = post(conn, tp_destination_rate_path(conn, :create), %{
      "meta" => %{},
      "data" => %{
  ...
  test "updates and renders chosen resource when data is valid", %{conn: conn} do
    ...
    conn = put(conn, tp_destination_rate_path(conn, :update, tp_destination_rate), %{
      "meta" => %{},
      "data" => %{
  ...
  test "deletes chosen resource", %{conn: conn} do
    ...
    conn = delete(conn, tp_destination_rate_path(conn, :delete, tp_destination_rate)) |> doc
  ...
  test "deletes chosen resource with assosiated resources included", %{conn: conn} do
    ...
    conn = delete(conn, tp_destination_rate_path(conn, :delete, tp_destination_rate))
    assert response(conn, 204)
  ...
  describe "DELETE delete_all" do
    test "deletes all records by filter", %{conn: conn}  do
      ...
      conn = conn
      |> post(tp_destination_rate_path(conn, :delete_all), %{tpid: tariff_plan.alias, filter: %{destinations_tag: "desttag2"}})
