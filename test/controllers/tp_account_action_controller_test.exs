defmodule CgratesWebJsonapi.TpAccountActionControllerTest do
  ...
  describe "GET index" do
    test "lists all entries related tariff plan on index", %{conn: conn} do
      ...
      conn = get(conn, tp_account_action_path(conn, :index, tpid: tariff_plan_1.alias)) |> doc
  ...
  describe "GET show" do
    test "shows chosen resource", %{conn: conn} do
      ...
      conn = get(conn, tp_account_action_path(conn, :show, tp_account_action)) |> doc
    ...
    test "does not show resource and instead throw error when id is nonexistent", %{conn: conn} do
      assert_error_sent 404, fn ->
        get(conn, tp_account_action_path(conn, :show, -1)) |> doc
      end
    end
  end
  ...
  describe "GET export_to_csv" do
    test "returns status 'ok'", %{conn: conn} do
      ...
      conn = conn
      |> get(tp_account_action_path(conn, :export_to_csv), %{tpid: tariff_plan.alias, filter: %{disabled: true, tenant: "t1"}})
      |> doc()
  ...
  describe "POST create" do
    test "creates and renders resource when data is valid", %{conn: conn} do
      ...
      conn = post(conn, tp_account_action_path(conn, :create), %{
        "meta" => %{},
        "data" => %{
  ...
  describe "PATCH/PUT update" do
    test "updates and renders chosen resource when data is valid", %{conn: conn} do
      ...
      conn = put(conn, tp_account_action_path(conn, :update, tp_account_action), %{
        "meta" => %{},
        "data" => %{
          "type" => "tp_account_action",
  ...
  describe "DELETE destroy" do
    test "deletes chosen resource", %{conn: conn} do
      ...
      conn = delete(conn, tp_account_action_path(conn, :delete, tp_account_action)) |> doc
  ...
  describe "DELETE delete_all" do
    test "deletes all records by filter", %{conn: conn}  do
      ...
      tp_account_action1 = insert :tp_account_action, tpid: tariff_plan.alias, disabled: true, tenant: "t1"
      tp_account_action2 = insert :tp_account_action, tpid: tariff_plan.alias, disabled: false

      conn = conn
      |> post(tp_account_action_path(conn, :delete_all), %{tpid: tariff_plan.alias, filter: %{disabled: false}})
