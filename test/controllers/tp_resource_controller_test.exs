defmodule CgratesWebJsonapi.TpResourceControllerTest do
  ...
  describe "GET index" do
    test "lists all entries related tariff plan on index", %{conn: conn} do
      ...
      conn = get(conn, tp_resource_path(conn, :index, tpid: tariff_plan_1.alias)) |> doc
  ...
  describe "GET show" do
    test "shows chosen resource", %{conn: conn} do
      ...
      conn = get(conn, tp_resource_path(conn, :show, tp_resource)) |> doc
    ...
    test "does not show resource and instead throw error when id is nonexistent", %{conn: conn} do
      assert_error_sent 404, fn ->
        get(conn, tp_resource_path(conn, :show, -1)) |> doc
      end
    end
  end
  ...
  describe "GET export_to_csv" do
    test "returns status 'ok'", %{conn: conn} do
      ...
      conn = conn
      |> get(tp_resource_path(conn, :export_to_csv), %{tpid: tariff_plan.alias, filter: %{blocker: true, tenant: "t1"}})
      |> doc()
  ...
  describe "POST create" do
    test "creates and renders resource when data is valid", %{conn: conn} do
      ...
      conn = post(conn, tp_resource_path(conn, :create), %{
        "meta" => %{},
        "data" => %{
          "type" => "tp_resource",
          "attributes" => params,
  ...
  describe "PATCH/PUT update" do
    test "updates and renders chosen resource when data is valid", %{conn: conn} do
      ...
      conn = put(conn, tp_resource_path(conn, :update, tp_resource), %{
        "meta" => %{},
        "data" => %{
          "type" => "tp_resource",
          "id" => tp_resource.pk,
  ...
  describe "DELETE destroy" do
    test "deletes chosen resource", %{conn: conn} do
      ...
      conn = delete(conn, tp_resource_path(conn, :delete, tp_resource)) |> doc
  ...
  describe "DELETE delete_all" do
    test "deletes all records by filter", %{conn: conn}  do
      ...
      conn = conn
      |> post(tp_resource_path(conn, :delete_all), %{tpid: tariff_plan.alias, filter: %{blocker: false}})
