defmodule CgratesWebJsonapi.TariffPlans.TpAttributeControllerTest do
  ...
  describe "GET index" do
    ...
    test "filtering by tenant", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_attribute_path(conn, :index, tpid: tariff_plan.alias), filter: %{tenant: t1.tenant})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by custom_id", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_attribute_path(conn, :index, tpid: tariff_plan.alias), filter: %{custom_id: t1.custom_id})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by contexts", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_attribute_path(conn, :index, tpid: tariff_plan.alias), filter: %{contexts: t1.contexts})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by filter_ids", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_attribute_path(conn, :index, tpid: tariff_plan.alias), filter: %{filter_ids: t1.filter_ids})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by attribute_filter_ids", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_attribute_path(conn, :index, tpid: tariff_plan.alias), filter: %{attribute_filter_ids: t1.attribute_filter_ids})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by path", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_attribute_path(conn, :index, tpid: tariff_plan.alias), filter: %{path: t1.path})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by cg_type", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_attribute_path(conn, :index, tpid: tariff_plan.alias), filter: %{cg_type: t1.cg_type})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by value", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_attribute_path(conn, :index, tpid: tariff_plan.alias), filter: %{value: t1.value})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by activation_interval", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_attribute_path(conn, :index, tpid: tariff_plan.alias), filter: %{activation_interval: t1.activation_interval})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by blocker", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_attribute_path(conn, :index, tpid: tariff_plan.alias), filter: %{blocker: true})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by weight", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_attribute_path(conn, :index, tpid: tariff_plan.alias), filter: %{weight: t1.weight})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
  end
  ...
  describe "GET export_to_csv" do
    test "returns status 'ok'", %{conn: conn} do
      ...
      conn = conn
      |> get(Routes.tp_attribute_path(conn, :export_to_csv), %{tpid: tariff_plan.alias, filter: %{blocker: true, tenant: "t1"}})
      |> doc()
  ...
  describe "POST create" do
    test "creates and renders resource when data is valid", %{conn: conn} do
      ...
      conn = post(conn, Routes.tp_attribute_path(conn, :create), %{
        "meta" => %{},
        "data" => %{
          "type" => "tp_attribute",
          "attributes" => params,
  ...
  describe "PATCH/PUT update" do
    test "updates and renders chosen resource when data is valid", %{conn: conn} do
      ...
      conn = put(conn, Routes.tp_attribute_path(conn, :update, tp_attribute), %{
        "meta" => %{},
        "data" => %{
          "type" => "tp_attribute",
          "id" => tp_attribute.pk,
  ...
  describe "DELETE delete_all" do
    test "deletes all records by filter", %{conn: conn}  do
      ...
      tp_attribute1 = insert :tp_attribute, tpid: tariff_plan.alias, blocker: true, contexts: "field1"
      tp_attribute2 = insert :tp_attribute, tpid: tariff_plan.alias, blocker: false

      conn = conn
      |> post(Routes.tp_attribute_path(conn, :delete_all), %{tpid: tariff_plan.alias, filter: %{blocker: false}})
