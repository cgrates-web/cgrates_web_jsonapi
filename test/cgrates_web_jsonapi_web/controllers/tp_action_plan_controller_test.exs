defmodule CgratesWebJsonapi.TariffPlans.TpActionPlanControllerTest do
  ...
  describe "GET index" do
    ...
    test "filtering by tag", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_action_plan_path(conn, :index, tpid: tariff_plan.alias), filter: %{tag: t1.tag})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by actions_tag", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_action_plan_path(conn, :index, tpid: tariff_plan.alias), filter: %{actions_tag: t1.actions_tag})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by timing_tag", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_action_plan_path(conn, :index, tpid: tariff_plan.alias), filter: %{timing_tag: t1.timing_tag})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by weight", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_action_plan_path(conn, :index, tpid: tariff_plan.alias), filter: %{weight: t1.weight})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
  end
  ...
  describe "GET export_to_csv" do
    test "returns status 'ok'", %{conn: conn} do
      ...
      conn = conn
      |> get(Routes.tp_action_plan_path(conn, :export_to_csv), %{tpid: tariff_plan.alias, filter: %{actions_tag: "at1", timing_tag: "t1"}})
      |> doc()
  ...
  describe "POST create" do
    test "creates and renders resource when data is valid", %{conn: conn} do
      ...
      conn = post(conn, Routes.tp_action_plan_path(conn, :create), %{
        "meta" => %{},
        "data" => %{
          "type" => "tp_action_plan",
          "attributes" => params,
  ...
  describe "PATCH/PUT update" do
    test "updates and renders chosen resource when data is valid", %{conn: conn} do
      ...
      conn = put(conn, Routes.tp_action_plan_path(conn, :update, tp_action_plan), %{
        "meta" => %{},
        "data" => %{
          "type" => "tp_action_plan",
  ...
  describe "DELETE delete_all" do
    test "deletes all records by filter", %{conn: conn}  do
      ...
      tp_action_plan1 = insert :tp_action_plan, tpid: tariff_plan.alias, actions_tag: "at1", timing_tag: "t1"
      tp_action_plan2 = insert :tp_action_plan, tpid: tariff_plan.alias, actions_tag: "at2"

      conn = conn
      |> post(Routes.tp_action_plan_path(conn, :delete_all), %{tpid: tariff_plan.alias, filter: %{actions_tag: "at1", timing_tag: "t1"}})
