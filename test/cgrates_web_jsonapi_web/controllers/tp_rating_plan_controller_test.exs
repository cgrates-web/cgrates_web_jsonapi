defmodule CgratesWebJsonapi.TariffPlans.TpRatingPlanControllerTest do
  ...
  test "filtering by tag", %{conn: conn} do
    ...
    conn = get(conn, Routes.tp_rating_plan_path(conn, :index, tpid: tariff_plan.alias), filter: %{tag: t1.tag})
    |> doc
    assert length(json_response(conn, 200)["data"]) == 1
  end
  ...
  test "filtering by destrates_tag", %{conn: conn} do
    ...
    conn = get(conn, Routes.tp_rating_plan_path(conn, :index, tpid: tariff_plan.alias), filter: %{destrates_tag: t1.destrates_tag})
    |> doc
    assert length(json_response(conn, 200)["data"]) == 1
  end
  ...
  test "filtering by timing_tag", %{conn: conn} do
    ...
    conn = get(conn, Routes.tp_rating_plan_path(conn, :index, tpid: tariff_plan.alias), filter: %{timing_tag: t1.timing_tag})
    |> doc
    assert length(json_response(conn, 200)["data"]) == 1
  end
  ...
  test "filtering by weight", %{conn: conn} do
    ...
    conn = get(conn, Routes.tp_rating_plan_path(conn, :index, tpid: tariff_plan.alias), filter: %{weight: 1.0})
    |> doc
    assert length(json_response(conn, 200)["data"]) == 1
  end
  ...
  describe "GET export_to_csv" do
    test "returns status 'ok'", %{conn: conn} do
      ...
      conn = conn
      |> get(Routes.tp_rating_plan_path(conn, :export_to_csv), %{tpid: tariff_plan.alias, filter: %{timing_tag: "timing_2"}})
      |> doc()
  ...
  test "creates and renders resource when data is valid", %{conn: conn} do
    ...
    conn = post(conn, Routes.tp_rating_plan_path(conn, :create), %{
      "meta" => %{},
      "data" => %{
        "type" => "tp_rating_plan",
        "attributes" => params,
  ...
  test "updates and renders chosen resource when data is valid", %{conn: conn} do
    ...
    conn = put(conn, Routes.tp_rating_plan_path(conn, :update, tp_rating_plan), %{
      "meta" => %{},
      "data" => %{
        "type" => "tp_rating_plan",
  ...
  describe "DELETE delete_all" do
    test "deletes all records by filter", %{conn: conn}  do
      ...
      conn = conn
      |> post(Routes.tp_rating_plan_path(conn, :delete_all), %{tpid: tariff_plan.alias, filter: %{timing_tag: "timing_2"}})
