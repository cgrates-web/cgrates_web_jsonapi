defmodule CgratesWebJsonapi.TariffPlans.TpStatControllerTest do
  ...
  describe "GET index" do
    ...
    test "filtering by custom_id", %{conn: conn} do
      ...
      conn =
        get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias),
          filter: %{custom_id: "c1"}
        )
        |> doc()
    ...
    test "filtering by ttl", %{conn: conn} do
      ...
      conn =
        get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{ttl: "a"})
        |> doc()
  ...
  describe "GET export_to_csv" do
    test "returns status 'ok'", %{conn: conn} do
      ...
      conn =
        conn
        |> get(Routes.tp_stat_path(conn, :export_to_csv), %{
          tpid: tariff_plan.alias,
          filter: %{custom_id: "cust_1", blocker: true}
        })
        |> doc()
  ...
  describe "POST create" do
    test "creates and renders resource when data is valid", %{conn: conn} do
      ...
      conn =
        post(conn, Routes.tp_stat_path(conn, :create), %{
          "meta" => %{},
          "data" => %{
            "type" => "tp_stat",
            "attributes" => params
  ...
  describe "PUT update" do
    test "updates and renders chosen resource when data is valid", %{conn: conn} do
      ...
      conn =
        put(conn, Routes.tp_stat_path(conn, :update, tp_stat), %{
          "meta" => %{},
          "data" => %{
            "type" => "tp_stat",
            "id" => tp_stat.pk,
  ...
  describe "DELETE delete_all" do
    test "deletes all records by filter", %{conn: conn} do
      ...
      conn =
        conn
        |> post(Routes.tp_stat_path(conn, :delete_all), %{
          tpid: tariff_plan.alias,
          filter: %{custom_id: "cust_2"}
        })
