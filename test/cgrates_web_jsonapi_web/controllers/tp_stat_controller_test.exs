defmodule CgratesWebJsonapi.TariffPlans.TpStatControllerTest do
  ...
  describe "GET index" do
    test "lists all entries related tariff plan on index", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan_1.alias)) |> doc()
    ...
    test "filtering by custom_id", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{custom_id: "c1"})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by filter_ids", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{filter_ids: "f1"})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by activation_interval", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{activation_interval: "a"})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by ttl", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{ttl: "a"})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by tenant", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{tenant: "t1"})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by min_items", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{min_items: 1})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by queue_length", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{queue_length: 1})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by threshold_ids", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{threshold_ids: "tid1"})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by metric_ids", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{metric_ids: "1"})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by metric_filter_ids", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{metric_filter_ids: "1"})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by blocker", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{blocker: true})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by stored", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{stored: true})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by weight", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :index, tpid: tariff_plan.alias), filter: %{weight: 100})
      |> doc()
      assert length(json_response(conn, 200)["data"]) == 1
    end
  end
  ...
  describe "GET show" do
    test "shows chosen resource", %{conn: conn} do
      ...
      conn = get(conn, Routes.tp_stat_path(conn, :show, tp_stat)) |> doc()
    ...
    test "does not show resource and instead throw error when id is nonexistent", %{conn: conn} do
      assert_error_sent 404, fn ->
        get(conn, Routes.tp_stat_path(conn, :show, -1)) |> doc
      end
    end
  end
  ...
  describe "GET export_to_csv" do
    test "returns status 'ok'", %{conn: conn} do
      ...
      conn = conn
      |> get(Routes.tp_stat_path(conn, :export_to_csv), %{tpid: tariff_plan.alias, filter: %{custom_id: "cust_1", blocker: true}})
      |> doc()
  ...
  describe "POST create" do
    test "creates and renders resource when data is valid", %{conn: conn} do
      ...
      conn = post(conn, Routes.tp_stat_path(conn, :create), %{
        "meta" => %{},
        "data" => %{
          "type" => "tp_stat",
          "attributes" => params
  ...
  describe "PUT update" do
    test "updates and renders chosen resource when data is valid", %{conn: conn} do
      ...
      conn = put(conn, Routes.tp_stat_path(conn, :update, tp_stat), %{
        "meta" => %{},
        "data" => %{
          "type" => "tp_stat",
          "id" => tp_stat.pk,
  ...
  describe "DELETE destroy" do
    test "deletes chosen resource", %{conn: conn} do
      tp_stat = insert :tp_stat
      conn = delete(conn, Routes.tp_stat_path(conn, :delete, tp_stat)) |> doc()
  ...
  describe "DELETE delete_all" do
    test "deletes all records by filter", %{conn: conn}  do
      ...
      conn = conn
      |> post(Routes.tp_stat_path(conn, :delete_all), %{tpid: tariff_plan.alias, filter: %{custom_id: "cust_2"}})
