defmodule CgratesWebJsonapi.TariffPlans.TpRatingProfileControllerTest do
  ...
  describe "GET index" do
    ...
    test "filtering by loadid", %{conn: conn} do
      ...
      conn = conn
      |> get(Routes.tp_rating_profile_path(conn, :index, tpid: tariff_plan.alias), filter: %{loadid: t1.loadid})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by tenant", %{conn: conn} do
      ...
      conn = conn
      |> get(Routes.tp_rating_profile_path(conn, :index, tpid: tariff_plan.alias), filter: %{tenant: t1.tenant})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by category", %{conn: conn} do
      ...
      conn = conn
      |> get(Routes.tp_rating_profile_path(conn, :index, tpid: tariff_plan.alias), filter: %{category: t1.category})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
    ...
    test "filtering by subject", %{conn: conn} do
      ...
      conn = conn
      |> get(Routes.tp_rating_profile_path(conn, :index, tpid: tariff_plan.alias), filter: %{subject: t1.subject})
      |> doc
      assert length(json_response(conn, 200)["data"]) == 1
    end
  ...
  describe "GET export_to_csv" do
    test "returns status 'ok'", %{conn: conn} do
      ...
      conn = conn
      |> get(Routes.tp_rating_profile_path(conn, :export_to_csv), %{tpid: tariff_plan.alias, filter: %{subject: "subj1"}})
      |> doc()
  ...
  describe "POST create" do
    test "creates and renders resource when data is valid", %{conn: conn} do
      ...
      conn = post(conn, Routes.tp_rating_profile_path(conn, :create), %{
        "meta" => %{},
        "data" => %{
  ...
  describe "PATCH/PUT update" do
    test "updates and renders chosen resource when data is valid", %{conn: conn} do
      ...
      conn = put(conn, Routes.tp_rating_profile_path(conn, :update, tp_rating_profile), %{
        "meta" => %{},
        "data" => %{
          "type" => "tp_rating_profile",
  ...
  describe "DELETE delete_all" do
    test "deletes all records by filter", %{conn: conn}  do
      ...
      conn = conn
      |> post(Routes.tp_rating_profile_path(conn, :delete_all), %{tpid: tariff_plan.alias, filter: %{subject: "subj2"}})
