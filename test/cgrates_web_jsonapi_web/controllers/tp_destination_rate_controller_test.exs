defmodule CgratesWebJsonapi.TariffPlans.TpDestinationRateControllerTest do
  ...
  test "lists all entries related tariff plan on index", %{conn: conn} do
    ...
    conn =
      get(conn, Routes.tp_destination_rate_path(conn, :index, tpid: tariff_plan_1.alias)) |> doc
  ...
  test "filtering by tag", %{conn: conn} do
    ...
    conn =
      get(conn, Routes.tp_destination_rate_path(conn, :index, tpid: tariff_plan.alias),
        filter: %{tag: dr1.tag}
      )
      |> doc
  ...
  describe "GET export_to_csv" do
    test "returns status 'ok'", %{conn: conn} do
      ...
      conn =
        conn
        |> get(Routes.tp_destination_rate_path(conn, :export_to_csv), %{
          tpid: tariff_plan.alias,
          filter: %{destinations_tag: "desttag1", tag: "drtag1"}
        })
        |> doc()
  ...
  test "creates and renders resource when data is valid", %{conn: conn} do
    ...
    conn =
      post(conn, Routes.tp_destination_rate_path(conn, :create), %{
        "meta" => %{},
        "data" => %{
  ...
  test "updates and renders chosen resource when data is valid", %{conn: conn} do
    ...
    conn =
      put(conn, Routes.tp_destination_rate_path(conn, :update, tp_destination_rate), %{
        "meta" => %{},
        "data" => %{
  ...
  test "deletes chosen resource", %{conn: conn} do
    ...
    conn =
      delete(conn, Routes.tp_destination_rate_path(conn, :delete, tp_destination_rate)) |> doc
  ...
  test "deletes chosen resource with assosiated resources included", %{conn: conn} do
    ...
    tp_rating_plan =
      insert(:tp_rating_plan, tpid: tariff_plan.alias, destrates_tag: tp_destination_rate.tag)
  ...
  describe "DELETE delete_all" do
    test "deletes all records by filter", %{conn: conn} do
      ...
      tp_destination_rate2 =
        insert(:tp_destination_rate, tpid: tariff_plan.alias, destinations_tag: "desttag2")

      conn =
        conn
        |> post(Routes.tp_destination_rate_path(conn, :delete_all), %{
          tpid: tariff_plan.alias,
          filter: %{destinations_tag: "desttag2"}
        })
