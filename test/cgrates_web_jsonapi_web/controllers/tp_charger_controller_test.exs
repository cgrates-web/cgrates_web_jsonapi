defmodule CgratesWebJsonapi.TariffPlans.TpChargerControllerTest do
  ...
  describe "GET index" do
    ...
    test "filtering by tenant", %{conn: conn} do
      ...
      conn =
        get(conn, Routes.tp_charger_path(conn, :index, tpid: tariff_plan.alias),
    ...
    test "filtering by attribute_ids", %{conn: conn} do
      ...
      conn =
        get(conn, Routes.tp_charger_path(conn, :index, tpid: tariff_plan.alias),
          filter: %{attribute_ids: t1.attribute_ids}
        )
        |> doc
  ...
  describe "GET export_to_csv" do
    test "returns status 'ok'", %{conn: conn} do
      ...
      conn =
        conn
        |> get(Routes.tp_charger_path(conn, :export_to_csv), %{
          tpid: tariff_plan.alias,
          filter: %{run_id: "runid1", tenant: "t1"}
        })
        |> doc()
  ...
  describe "POST create" do
    test "creates and renders resource when data is valid", %{conn: conn} do
      ...
      conn =
        post(conn, Routes.tp_charger_path(conn, :create), %{
          "meta" => %{},
          "data" => %{
            "type" => "tp_charger",
            "attributes" => params
  ...
  describe "PATCH/PUT update" do
    test "updates and renders chosen resource when data is valid", %{conn: conn} do
      ...
      conn =
        put(conn, Routes.tp_charger_path(conn, :update, tp_charger), %{
          "meta" => %{},
          "data" => %{
            "type" => "tp_charger",
            "id" => tp_charger.pk,
  ...
  describe "DELETE delete_all" do
    test "deletes all records by filter", %{conn: conn} do
      ...
      tp_charger1 =
        insert(:tp_charger, tpid: tariff_plan.alias, run_id: "runid1", tenant: "tenant1")

      tp_charger2 = insert(:tp_charger, tpid: tariff_plan.alias, run_id: "runid2")

      conn =
        conn
        |> post(Routes.tp_charger_path(conn, :delete_all), %{
          tpid: tariff_plan.alias,
          filter: %{run_id: "runid2"}
        })
