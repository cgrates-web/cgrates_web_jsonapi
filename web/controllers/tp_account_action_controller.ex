defmodule CgratesWebJsonapi.TpAccountActionController do
  ...
  def filter(_conn, query, "action_plan_tag", val),     do: query |> where([r], like(r.action_plan_tag, ^"%#{val}%"))
  def filter(_conn, query, "action_triggers_tag", val), do: query |> where([r], like(r.action_triggers_tag, ^"%#{val}%"))
