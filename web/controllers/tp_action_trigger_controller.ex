defmodule CgratesWebJsonapi.TpActionTriggerController do
  ...
  def filter(_conn, query, "actions_tag", val),              do: query |> where([r], like(r.actions_tag, ^"%#{val}%"))
  ...
  def filter(_conn, query, "balance_directions", val),       do: query |> where(balance_directions: ^val)
  def filter(_conn, query, "unique_id", val),                do: query |> where([r], like(r.unique_id, ^"%#{val}%"))
  ...
  def filter(_conn, query, "min_queued_items", val),         do: query |> where(min_queued_items: ^val)
  def filter(_conn, query, "expiry_time", val),              do: query |> where(expiry_time: ^val)
  def filter(_conn, query, "balance_expiry_time", val),      do: query |> where(balance_expiry_time: ^val)
  def filter(_conn, query, "activation_time", val),          do: query |> where(activation_time: ^val)
  def filter(_conn, query, "balance_timing_tags", val),      do: query |> where([r], like(r.balance_timing_tags, ^"%#{val}%"))
  def filter(_conn, query, "balance_destination_tags", val), do: query |> where([r], like(r.balance_destination_tags, ^"%#{val}%"))
  def filter(_conn, query, "balance_rating_subject", val),   do: query |> where([r], like(r.balance_rating_subject, ^"%#{val}%"))
  def filter(_conn, query, "balance_categories", val),       do: query |> where([r], like(r.balance_categories, ^"%#{val}%"))
  def filter(_conn, query, "balance_shared_groups", val),    do: query |> where([r], like(r.balance_shared_groups, ^"%#{val}%"))
