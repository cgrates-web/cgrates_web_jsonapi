defmodule CgratesWebJsonapi.TpRatingProfileController do
  ...
  def filter(_conn, query, "loadid", loadid),        do: query |> where([r], like(r.loadid, ^"%#{loadid}%"))
  def filter(_conn, query, "direction", direction),  do: query |> where([r], like(r.direction, ^"%#{direction}%"))
  def filter(_conn, query, "tenant", tenant),        do: query |> where([r], like(r.tenant, ^"%#{tenant}%"))
  def filter(_conn, query, "category", category),    do: query |> where([r], like(r.category, ^"%#{category}%"))
  def filter(_conn, query, "subject", subject),      do: query |> where([r], like(r.subject, ^"%#{subject}%"))
  def filter(_conn, query, "activation_time", at),   do: query |> where([r], like(r.activation_time, ^"%#{at}%"))
